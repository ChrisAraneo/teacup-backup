...
export class FileSystemMock extends FileSystem {
  ...
  readFile(path: string, _options, callback: (error: any, data?: any) => any): void {
    if (this.isCorrectTextFile(path)) {
      ...
      callback(
        null,
        `{"backupDirectory":".\/backups","files":[{"filename":"index.ts"}],"ftp":{"directory":"mini-backup\/","enabled":true,"host":"192.168.50.1","password":"Qwerty123\/","user":"user"},"interval":3600,"log-level":"debug","mode":"backup","roots":["root"]}`,
      );
